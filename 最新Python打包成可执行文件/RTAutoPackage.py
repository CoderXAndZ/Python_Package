...
def archive_project(project_path, configuration, password, method):
    ...
    # 如果打企业版的包，替换掉描述文件
    if method == "enterprise":
        change_profile(archive_path, "embedded_Enterprise.mobileprovision")
    else:
        change_profile(archive_path, "embedded_AppStore.mobileprovision")
...
def change_profile(archive_path, profile_path):
    ...
    # rtyx_profile = os.path.abspath(profile_path)   # 使用这句不能在使用pyinstall打包之后获取文件路径
    rtyx_profile = os.path.dirname(os.path.realpath(sys.argv[0]))[0:-4] + profile_path
    print("rtyx_profile:", rtyx_profile)
    # 目的路径
    ...
    # /Users/admin/Library/Developer/Xcode/Archives/2019-02-14/RTYX 2019-2-14 下午3.23.xcarchive/Products/Applications/RTYX.app/embedded_AppStore.mobileprovision
    # 当前目录 "/Users/admin/Documents/长久保存/Python打包/最新Python打包成可执行文件"
    # "/Users/admin/Documents/长久保存/Python打包/最新Python打包成可执行文件/RTYXEnterpriseProfile/embedded_AppStore.mobileprovision"


# 打印路径信息
"""
print("path[0] 获取到的路径是：", sys.path[0])
print("argv[0] 获取到的路径是：", sys.argv[0])
print("dirname[0] 获取到的路径是：", os.path.dirname(os.path.realpath(sys.executable)))
print("dirname[1] 获取到的路径是：", os.path.dirname(os.path.realpath(sys.argv[0])))
"""
