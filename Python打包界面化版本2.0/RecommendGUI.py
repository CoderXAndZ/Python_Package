...
def judgePath(excelFile, projectFile, outputPath, infoPlistPath):
    if excelFile != '' and projectFile != '' and outputPath != '' and infoPlistPath != '':
        ...
        messagebox.showinfo("温馨提示", "请选择ipa存放路径！" )
    else:
...
# tk.Button(root, text = "开始打包", command = lambda:packPkg(excelFile.get(), projectFile.get(), outputPath.get(), infoPlistPath.get())).grid(row = 4, column = 0, pady = 10)
tk.Button(root, text = "开始打包", command = lambda:packPkg("/Users/admin/Desktop/工作簿1.xlsx","/Users/admin/ios/fmapp.xcodeproj","/Users/admin/Desktop/生成的ipa包", infoPlistPath.get())).grid(row = 4, column = 0, pady = 10)
# 推荐人20180301  工作簿1
