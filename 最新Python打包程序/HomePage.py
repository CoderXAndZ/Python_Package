...
def setup_view():
    ...
    tk.Button(root, text="融托金融批量打包", command=lambda: choose_application(1)).grid(row=0, column=1, padx=10, pady=5)

    # 融托优选
    tk.Button(root, text="融托优选", command=lambda: choose_application(2)).grid(row=2, column=0, padx=10, pady=5)
    tk.Button(root, text="融托优选批量打包", command=lambda: choose_application(3)).grid(row=2, column=1, padx=10, pady=5)

    # 融米小管家
    tk.Button(root, text="融米小管家", command=lambda: choose_application(4)).grid(row=3, column=0, padx=10, pady=5)
