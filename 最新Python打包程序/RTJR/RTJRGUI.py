...
from RTJR import RTJRAutoPackage   # 融托金融自动打包
...
def create_window(application_name):
    ...
    # ipa包输出路径
    ttk.Label(root, text="输出路径:").grid(row=1, column=0)
    ttk.Label(root, text="ipa包输出路径, 默认路径是-Desktop/RTJRipa").grid(row=1, column=1)

    ttk.Label(root, text="参数设置:").grid(row=2, column=0)

    # 让用户选择 method - 打包的类型
    global method  # 打包方法：ad-hoc、enterprise、app-store
    method = StringVar()
    array = [' ', 'ad-hoc', 'enterprise', 'app-store']
    method.set(array[1])
    option_method = ttk.OptionMenu(root, method, *array).grid(row=2, column=1, pady=10)
    ...
    option_scheme = ttk.OptionMenu(root, scheme_global, ' ', 'rongtuojinrong', 'rongtuojinrongQY').grid(row=2, column=2, pady=10)
    ...
    option_configuration = ttk.OptionMenu(root, configuration, ' ',  'Debug', 'Release')
    ...
    # 打包
    ttk.Button(root, text="开始打包", command=lambda: judge_path(project_file.get())).grid(row=3, column=1, pady=10)
